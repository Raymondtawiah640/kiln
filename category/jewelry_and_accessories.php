...
// Get the category ID for "Jewelry & Accessories"
try {
    $stmt = $pdo->prepare("SELECT id FROM categories WHERE name = ?");
    $stmt->execute(['Jewelry & Accessories']);
    $category = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$category) {
        die("Jewelry & Accessories category not found.");
    }
...
<div class="card" style="margin-top: 2rem;">
    <div class="flash-sales-heading" style="justify-content: space-between;">
        <div><i class="fas fa-gem"></i> Jewelry & Accessories</div>
        <a class="see-all" href="category_products.php?category_id=<?= $categoryId ?>">See All &raquo;</a>
    </div>
    <div class="flash-scroll-wrapper">
        <button class="flash-arrow left" onclick="scrollFlash('left', 'jewelry-scroll')" aria-label="Scroll left"><i class="fas fa-chevron-left"></i></button>
        <div class="flash-scroll-container" id="jewelry-scroll">
        ...
        <button class="flash-arrow right" onclick="scrollFlash('right', 'jewelry-scroll')" aria-label="Scroll right"><i class="fas fa-chevron-right"></i></button>
    </div>
</div>
...
// Optional: Add touch support for jewelry-scroll container (like flash sales)
document.addEventListener('DOMContentLoaded', function() {
    const containers = ['flash-scroll', 'jewelry-scroll'];
