...
// Get the category ID for "Smart Home Devices"
try {
    $stmt = $pdo->prepare("SELECT id FROM categories WHERE name = ?");
    $stmt->execute(['Smart Home Devices']);
    $category = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$category) {
        die("Smart Home Devices category not found.");
    }
...
<div class="card" style="margin-top: 2rem;">
    <div class="flash-sales-heading" style="justify-content: space-between;">
        <div><i class="fas fa-plug"></i> Smart Home Devices</div>
        <a class="see-all" href="category_products.php?category_id=<?= $categoryId ?>">See All &raquo;</a>
    </div>
    <div class="flash-scroll-wrapper">
        <button class="flash-arrow left" onclick="scrollFlash('left', 'smart-home-scroll')" aria-label="Scroll left"><i class="fas fa-chevron-left"></i></button>
        <div class="flash-scroll-container" id="smart-home-scroll">
        ...
        <button class="flash-arrow right" onclick="scrollFlash('right', 'smart-home-scroll')" aria-label="Scroll right"><i class="fas fa-chevron-right"></i></button>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    const containers = ['flash-scroll', 'smart-home-scroll'];
